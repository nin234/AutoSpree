...
-(void) updateDeletedItems
{
    ...
    for (NSUInteger j=0; j < ecnt; ++j)
    {
        ...
        {
            CarItem *item = [itemNamesTmp objectAtIndex:i];
...
-(void) refreshData
{
    ...
    NSEntityDescription *descr = [NSEntityDescription entityForName:@"CarItem" inManagedObjectContext:moc];
    ...
    switch (pDlg.sortIndx)
    {
        ...
        case 2:
        {
            NSSortDescriptor* ageDescriptor = [[NSSortDescriptor alloc] initWithKey:@"make"
                                                                          ascending:pDlg.bMakeAsc?YES:NO];
            NSArray* sortDescriptors = [NSArray arrayWithObject:ageDescriptor];
            itemNamesTmp = [[moc executeFetchRequest:req error:&error]sortedArrayUsingDescriptors:sortDescriptors];
        }
            break;
        ...
        case 4:
        {
            NSSortDescriptor* ageDescriptor = [[NSSortDescriptor alloc] initWithKey:@"model"
                                                                          ascending:pDlg.bModelAsc?YES:NO];
            NSArray* sortDescriptors = [NSArray arrayWithObject:ageDescriptor];
            itemNamesTmp = [[moc executeFetchRequest:req error:&error]sortedArrayUsingDescriptors:sortDescriptors];
        }
            break;
        case 5:
        {
            NSSortDescriptor* ageDescriptor = [[NSSortDescriptor alloc] initWithKey:@"miles"
                                                                          ascending:pDlg.bMilesAsc?YES:NO];
            NSArray* sortDescriptors = [NSArray arrayWithObject:ageDescriptor];
            itemNamesTmp = [[moc executeFetchRequest:req error:&error]sortedArrayUsingDescriptors:sortDescriptors];
        }
            break;
        ...
        case 6:
        {
            NSSortDescriptor* colorDescriptor = [[NSSortDescriptor alloc] initWithKey:@"color"
                                                                          ascending:pDlg.bColorAsc?YES:NO];
            NSArray* sortDescriptors = [NSArray arrayWithObject:colorDescriptor];
            itemNamesTmp = [[moc executeFetchRequest:req error:&error]sortedArrayUsingDescriptors:sortDescriptors];
        }
            break;
    ...
    for (int i=0 ; i < nItems; ++i)
    {
        ...
        bool add = false;
        struct stat sb;
        NSLog(@"iCloudSync=%d %@ %@\n", item.icloudsync, item.name, item.album_name);
