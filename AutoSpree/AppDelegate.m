...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"AutoSpree" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    unlocked = false;
    NSError *error = nil;
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"AutoSpree.sqlite"];
    NSUserDefaults* kvlocal = [NSUserDefaults standardUserDefaults];
    ...
    // iCloud content it may take a long long time to download
        ...
        // this needs to match the entitlements and provisioning profile
            ...
            NSLog(@"newTokenData %@", newTokenData);
                ...
                if (!bTokInArry)
                {
                    NSString *storeURLStr = [NSString stringWithFormat:@"AuotoSpree%d.sqlite", [tokArry count]];
                    storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:storeURLStr];
                    NSLog(@"Changing store URL to %@", storeURL);
            ...
            NSDictionary* options = [NSDictionary dictionaryWithObjectsAndKeys:@"com.rekhaninan.AutoSpree.cardatainfo", NSPersistentStoreUbiquitousContentNameKey, cloudURL, NSPersistentStoreUbiquitousContentURLKey, [NSNumber numberWithBool:YES], NSMigratePersistentStoresAutomaticallyOption, [NSNumber numberWithBool:YES], NSInferMappingModelAutomaticallyOption,nil];
            ...
            if (![psc addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:options error:&error]) 
            {
                ...
                if ([[dev systemVersion] doubleValue] >= 6.0)
                {
                  if (error.code == 134316 && [error.domain isEqualToString:@"NSCocoaErrorDomain"])
                  {
                    ...
                    [kvlocal setObject:newTokArry forKey:@"iCloudTokens"];
                    storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"AutoSpree1.sqlite"];
                    if (![psc addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:options error:&error])
                    {
                        bPersistError =true;
                        dispatch_async(dispatch_get_main_queue(), ^{
