...
-(void) photoSelDone
{
    ...
    NSLog(@"No of attchments  %d no of movOrImg %d original count attchments %d  movOrImg %d\n",  [attchments count], [movOrImg count],[albumContentsViewController.attchments count], [albumContentsViewController.movOrImg count]);
    AppDelegate *pDlg = (AppDelegate *)[[UIApplication sharedApplication] delegate];
...
-(void) getPhotos :(int) startIndx source:(int) source
{
    ...
    NSUInteger count = [seletedItems count];
    bool bFound = false;
    for (NSUInteger i =startIndx; i < count; ++i)
    {
        if ([[seletedItems objectAtIndex:i] boolValue] == YES)
        {
            currPhotoSelIndx = i;
...
-(NSString *) getMessage : (int) source
{
    ...
    message = [message stringByAppendingString:@"AutoSpree iPhone app https://itunes.apple.com/app/autospree/id559809146?ign-mpt=uo%3D5\n"];
    return message;
}
